...
export async function studentProfileData(userId) {
	try{
		...
		for(let i = 0;i<channelList.length;i++){
			...
			const rawChannelMessages = await client.conversations.history({token: process.env.TOKEN, channel: channelID, });
		    ...
		    for(let j = 0;j<channelMessages.length;j++){
				...
				if(isThereReply){
					let replyThread = await client.conversations.replies({token: process.env.TOKEN, channel: channelID, ts:threadTs });
			...
			var eachChannelStats = {id:channelID,channelName:channelName,totalMessagesForEachChannel:totalMessagesPerChannel,totalCallsForEachChannel:totalCallsForEachChannel}   
        ...
        return {traineeName:realName,profilePic:profilePic,finalTime:finalTime ,messagesStatsForEachChannel:messagesStatsForEachChannel,totalMessages:totalMessages,totalCalls:totalCalls }
